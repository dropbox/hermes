...
def test_very_large_note(sample_data1):
    """Test to ensure notes can be larger than 1024 bytes."""
    event_types = sample_data1.query(models.EventType).all()
    assert len(event_types) == 7
    event_type1 = event_types[0]
    ...
    very_large_note = "x" * 60000
    models.Event.create(
        sample_data1, host, "testman", event_type1, note=very_large_note
    )
    sample_data1.commit()
    ...
    assert event.user == "testman"
    assert event.event_type == event_type1
    assert event.note == very_large_note